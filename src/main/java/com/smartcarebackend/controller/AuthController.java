...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@RequestBody LoginRequest loginRequest) {
        User user=userRepository.findByUsername(loginRequest.getUsername()).orElseThrow(()->new RuntimeException("존재하지 않는 사용자입니다"));
        ...
        try {
            ...
            //잘못된 인증
            map.put("message", "로그인 정보가 일치하지 않습니다");
            //인증이 실패했다고 명시
    ...
    @Transactional
    public ResponseEntity<?> registerUser(@RequestParam(value = "userimage", required = false) MultipartFile userimage, @Valid @ModelAttribute SignupRequest signUpRequest) {
        System.out.println("싸인" + signUpRequest);
        System.out.println("싸인2" + userimage);
        ...
        //입력한 ssn이 이미 존재하는가
        if (userRepository.existsBySsn(signUpRequest.getSsn())) {
            return ResponseEntity.badRequest().body(new MessageResponse("해당 주민등록번호로 가입된 계정이 이미 존재합니다"));
        }
        //비밀번호와 확인용 비밀번호가 일치하는가
        if (!signUpRequest.getPassword().equals(signUpRequest.getConfirmPassword())) {
            return ResponseEntity.badRequest().body(new MessageResponse("확인용 비밀번호가 일치하지 않습니다"));
        }

        //이미지 저장 시작
        String fileName = "anyuser.png"; //기본 이미지의 파일명 할당
        //제출한 이미지가 있는 경우 이미지를 저장하고 이미지 파일명을 새롭게 할당
        if (userimage != null && !userimage.isEmpty()) {
            Date createDate = new Date(); //오늘 날짜
            fileName = createDate.getTime() + "_" + userimage.getOriginalFilename(); //파일이름 정의
            try {
                String uploadDir = "public/userimage/"; //업로드 폴더 경로 문자열
                Path uploadPath = Paths.get(uploadDir); //업로드 폴더 경로를 path타입으로 변환
                ...
                return ResponseEntity.badRequest().body(new MessageResponse("이미지 저장 오류"));
            }
        }
    ...
    public ResponseEntity<String> deleteUser(@RequestBody SignupRequest signUpRequest) {
        ...
        //입력한 비밀번호가 맞지 않으면 되돌아간다
        if (!passwordEncoder.matches(signUpRequest.getPassword(), user.getPassword())) {
            return ResponseEntity.badRequest().body("비밀번호가 일치하지 않습니다");
        }
        String userImage = user.getUserimage();
        System.out.println("이미지이름"+userImage);
        //이미지가 기본이미지(anyuser.png)가 아닐 경우에만 이미지를 삭제
        if(!userImage.equals("anyuser.png")) {
            try {
                String uploadDir = "public/userimage/"; //업로드 폴더 경로 문자열
                Path uploadPath = Paths.get(uploadDir+userImage); //업로드 폴더 경로를 path타입으로 변환
                System.out.println("이미지 경로"+uploadPath);
                if (Files.exists(uploadPath)) {
                    Files.delete(uploadPath);
                }else{
                    return ResponseEntity.badRequest().body("회원 이미지를 찾을 수 없습니다.");
                }
            } catch (Exception e) {
                return ResponseEntity.badRequest().body("회원탈퇴 중 이미지 삭제가 제대로 이루어지지 않았습니다");
            }
        }
